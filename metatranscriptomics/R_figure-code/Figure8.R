...
###-------- load data --------###
dta <- read.csv("input/Nmetabolismde.csv", header=TRUE, row.names = 1)
sd <- read.csv("input/Nmetabolismsd.csv", header=TRUE, row.names = 1)
turb <- read.csv("input/turbid.csv", header = TRUE, row.names = 1)
geneName <- read.csv("input/NmetabolismName.csv", header = TRUE, row.names = 1)

###-------- set colors and other formatting --------###
ann_colors = list(turbidity = c("firebrick", "yellow", "darkgreen"),
                  substrate = c("L-Glutamine->Glutamate"="#1B9E77" ,
                                "Nitrite->Ammonia"=  "#D95F02", 
                                "Nitrate->Nitrite"= "#7570B3" ,
                                "Ammonia->L-Glutamine" = "#E7298A",
                                "Hydroxylamine->Nitrite" = "#E6AB02",
                                "Ammonia->Hydroxylamine" = "#66A61E",
                                "Nitrite->Nitric oxide" = "#0072B2",
                                "Nitrate" = "purple",
                                "Ammonium" = "brown"))
